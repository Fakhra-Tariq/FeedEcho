...
import { Users, Key, BookOpen, Clock, Award, FileText } from 'lucide-react';
...
const AudienceJoinSession = () => {
  ...
  useEffect(() => {
    ...
    } else if (loggedInAudience?.name) {
      setStudentName(loggedInAudience.name);
    }
  ...
  }, [location.state, loggedInAudience?.name, loggedInAudience?.uid, loggedInAudience?.email]);
  ...
  const handleTeamSelect = async () => {
    ...
    try {
      await proceedWithSessionJoin({
        ...
        teamId,
        navigate,
        studentUid: pendingJoin.studentUid || loggedInAudience?.uid || null,
        studentEmail: pendingJoin.studentEmail || loggedInAudience?.email || null,
        loggedInAudience,
      });
    } catch (err) {
          ...
          <div className="bg-white rounded-3xl shadow-soft border border-primary/10 p-8">
            <div className="mb-6">
              ...
              <input
                ...
                readOnly={!!loggedInAudience?.name}
                className={`w-full px-4 py-3 border-2 border-neutral-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-colors text-text placeholder-neutral-400 ${
                  loggedInAudience?.name ? 'bg-gray-50' : ''
                }`}
                maxLength={50}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
...
export default AudienceJoinSession;
