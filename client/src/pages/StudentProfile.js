...
export default function StudentProfile() {
  ...
  const { studentLogout, updateUserProfile, changeUserPassword, userProfile } = useAuth();
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (result?.success && result.user) {
          setProfile(mapUserToProfile(result.user, getStoredStudentSession()));
        }
      } catch (error) {
  ...
  const handleEditSave = async () => {
    ...
    persistStudentSession({
      uid: getStoredStudentSession()?.uid,
      email: profile.email,
      firstName,
      lastName,
