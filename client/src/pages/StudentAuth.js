...
const StudentAuth = () => {
  ...
  const { studentSignIn, studentSignInWithGoogle } = useAuth();
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    if (!validateLogin() || loading || isSubmitting) return;
    ...
    try {
      const result = await studentSignIn(loginData.email.trim(), loginData.password);
      if (result?.success) {
        navigate('/student/home');
      } else {
  ...
  const onGoogle = async () => {
    ...
    try {
      const result = await studentSignInWithGoogle();
      if (result?.success) {
        navigate('/student/home');
      } else {
      ...
      const apiError = handleAPIError(err);
      setErrors({ general: apiError.message || 'Google sign-in failed. Please try again.' });
    } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-10 bg-background">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        <h1 className="text-2xl font-bold text-text mb-2">Student Login</h1>
        <p className="text-text-light mb-6">Sign in to your student account.</p>
        ...
        <div className="mt-4 flex justify-between text-sm">
          <Link to="/student/signup" className="text-primary hover:text-primary-dark font-medium">
            Create account
          </Link>
          <Link to="/student/forgot" className="text-primary hover:text-primary-dark font-medium">
            Forgot password?
          </Link>
        </div>
      </div>
