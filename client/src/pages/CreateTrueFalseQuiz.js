...
const CreateTrueFalseQuiz = () => {
  ...
  useEffect(() => {
    ...
    if (quiz && normalizeQuizTypeLabel(quiz.type) === 'True / False') {
  ...
  const addQuestion = () => {
    const newQuestion = {
      id: Date.now(),
      questionText: '',
      correctAnswer: null // null, 'true', or 'false'
    };
    setQuestions([...questions, newQuestion]);
    // Reset save state when adding new questions
    setIsQuizSaved(false);
  };

  const removeQuestion = (questionId) => {
    setQuestions(questions.filter(q => q.id !== questionId));
    // Reset save state when removing questions
    setIsQuizSaved(false);
  };
  ...
  const updateQuestionText = (questionId, text) => {
    ...
    // Reset save state when updating questions
    setIsQuizSaved(false);
  };
  ...
  const setCorrectAnswer = (questionId, answer) => {
    ...
    // Reset save state when changing answers
    setIsQuizSaved(false);
  };
  ...
  const saveQuiz = async () => {
    ...
    const validQuestions = questions.every(q => q.questionText.trim() && q.correctAnswer !== null);
    if (!validQuestions) {
      alert.toast.error('Please complete all questions with text and select a correct answer.');
      return;
    }
    ...
    try {
      const res = isActuallyEditMode
        ? await quizzesAPI.update(editingQuizId, payload)  // Fixed: was editingQuizId typo
          ...
          <div className="max-w-3xl">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Reset save state when changing title
                setIsQuizSaved(false);
              }}
            ...
            <div key={question.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 sm:p-4 lg:p-5">
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {/* True Option */}
                  <button
                    onClick={() => setCorrectAnswer(question.id, 'true')}
                  ...
                  {/* False Option */}
                  <button
                    onClick={() => setCorrectAnswer(question.id, 'false')}
                    className={`flex items-center space-x-3 px-4 py-3 rounded-xl border-2 transition-all duration-200 font-medium text-sm sm:text-base ${
            ...
            <div className="text-center py-4 sm:py-6">
              <div className="bg-orange-50 rounded-2xl p-3 sm:p-4 max-w-md mx-auto">
                ...
                <p className="text-xs sm:text-sm text-gray-600 mb-3">
                  Click "Add Question" to start creating your true/false quiz
                </p>
                <button
                  onClick={addQuestion}
