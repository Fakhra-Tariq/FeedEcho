...
export default function TeacherSpaceRace() {
  const { alert: hybridAlert } = useHybridAlert();
  const { data: teacherData, updateSpaceRace, logActivity } = useTeacherData();
  ...
  const [launchSettings, setLaunchSettings] = useState({
    ...
    showFinalScore: true,
    studentsPerTeam: 3 // Default 3 students per team for student choice
  });
  ...
  const handleOpenCreateModal = () => {
    ...
    setLaunchSettings({
      ...
      showFinalScore: true,
      studentsPerTeam: 3 // Reset students per team
    });
    setShowCreate(true);
    ...
    try {
      ...
      if (race.status === 'draft') {
        ...
        if (response.data.success) {
          ...
          // Stay on the Space Race page (this app doesn't have a /teacher/space-race/:id route)
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center space-x-2 ml-4">
                ...
                {(race.status === 'completed' || race.status === 'ended' || race.status === 'active') && (
                  <button 
                    onClick={() => navigate(`/teacher/space-race/${race.id}/display`)}
                    className="p-2 text-[#6D415F] hover:bg-[#6D415F]/10 rounded-lg transition-colors"
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <label className="flex items-center space-x-3">
                          <input
                            ...
                            className="w-4 h-4 text-[#6D415F] focus:ring-[#6D415F]"
                          />
                          <span className="text-sm text-text">Student Choice</span>
                        </label>
                      </div>
                    </div>

                    {/* Show students per team field only when student choice is selected */}
                    {launchSettings.teamAssignment === 'student-choice' && (
                      <div>
                        <label className="block text-sm font-medium text-text-light mb-1">Number of students per team</label>
                        <select
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">Students will see this timer when they attempt quiz</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">Students can join for this duration</p>
                      </div>
                    </div>
                  </div>
