...
import { useHostRealtimeRevision } from './HostRealtimeSyncContext';
...
/** Map Firebase activeSession/singleton to HostDataContext.activeSession shape */
...
const HostDataContext = createContext(null);
...
export const HostDataProvider = ({ children }) => {
  ...
  const dataRevision = useHostRealtimeRevision();
  ...
  return (
    <HostDataContext.Provider value={value}>{children}</HostDataContext.Provider>
  );
};
...
export const useHostData = () => {
  ...
  if (!context) {
    throw new Error('useHostData must be used within a HostDataProvider');
  }
  return context;
};
