...
const HostRealtimeSyncContext = createContext(0);
 ...
 */
export function HostRealtimeSyncProvider({ children }) {
  ...
  return (
    <HostRealtimeSyncContext.Provider value={revision}>{children}</HostRealtimeSyncContext.Provider>
  );
}

export function useHostRealtimeRevision() {
  return useContext(HostRealtimeSyncContext);
}
